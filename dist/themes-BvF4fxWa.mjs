//#region foundation/color.ts
/** Parse oklch(L C H) or oklch(L C H / alpha) → components. */
function parseOklchParts(s) {
...
function oklchToHexInner(L, C, H, alpha) {
...
*/
function resolveSizeToken(token, dim, kind) {
	const dimTable = SIZE_SCALE[dim];
	if (!dimTable) return .015;
	const kindTable = dimTable[kind ?? ""] ?? dimTable.arrow ?? dimTable.sphere;
	if (!kindTable) return .015;
	return kindTable[token] ?? kindTable.thin ?? kindTable.small ?? .015;
}
const IS_COLOR = new Set(COLOR_NAMES);
/**
...
const alpha = (c, pct = 15) => {
	return `color-mix(in oklab, ${resolveColor(c)} ${pct}%, transparent)`;
};
...
//#endregion
export { palette as a, svgColor as c, alpha as i, themes as n, resolveSizeToken as o, TOKENS as r, oklchToHex as s, resolveTheme as t };
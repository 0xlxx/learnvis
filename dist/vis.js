...
//#region vis/create.js
	const create = (selector, { width = 560, height = 400, margin = 48, geom: { nW = 34, nH = 26, dR = 8, rx = 5, gap = 4 } = {} } = {}) => {
		...
		const smartLabel = (anchorDecl, text, opts = {}) => {
			...
			const vb = C.svg.node().viewBox.baseVal;
			const plain = String(text).replace(/<[^>]+>/g, "");
			const estW = Math.max(plain.length * size * .65 + 16, 50);
			const estH = size + 10;
			...
			if (!best && sides.length) {
				...
				for (let e = 0; e < 250; e += 12) {
					const r = {
						x: pos.x + ox + dx * e - estW / 2,
						y: pos.y + oy + dy * e - estH / 2,
						w: estW,
						h: estH
					};
					if (!collidesAny(r, placedLabels) && !collidesAny(r, obstacles)) {
			...
			const div = C.root.append("div").attr("class", className).style("position", "absolute").style("pointer-events", "none").style("left", lx + "%").style("top", ly + "%").style("transform", "translate(-50%, -100%)").html(text);
			for (const [k, v] of Object.entries(style)) div.style(k, v);
			return div;
		};
